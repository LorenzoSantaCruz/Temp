...
namespace UE::InstancedActors
{
	...
	template <>
	bool PassesBoundsTest<FSphere>(const FSphere& QueryBounds, EBoundsTestType BoundsTestType, const FInstancedActorsInstanceHandle& InstanceHandle, const FTransform& InstanceTransform)
	{
	...
	template <>
	bool PassesBoundsTest<FBox>(const FBox& QueryBounds, EBoundsTestType BoundsTestType, const FInstancedActorsInstanceHandle& InstanceHandle, const FTransform& InstanceTransform)
	{
		switch (BoundsTestType)
		{
		case EBoundsTestType::Intersect:
...
void AInstancedActorsManager::SetupLoadedInstances()
{
...
UInstancedActorsData* AInstancedActorsManager::CreateNextInstanceActorData(TSubclassOf<AActor> ActorClass, const FInstancedActorsTagSet& AdditionalInstanceTags)
{
	...
	const FName UniqueName = MakeUniqueObjectName(this, InstancedActorsDataClass, FName(InstanceDataNameStr));
	UInstancedActorsData* NewInstanceData = NewObject<UInstancedActorsData>(this, InstancedActorsDataClass, UniqueName);
...
UInstancedActorsData& AInstancedActorsManager::GetOrCreateActorInstanceData(TSubclassOf<AActor> ActorClass, const FInstancedActorsTagSet& AdditionalInstanceTags, bool bCreateEditorPreviewISMCs)
{
	checkf(HasActorBegunPlay() == false, TEXT("AInstancedActorsManager doesn't yet support runtime addition of instances"));

	TObjectPtr<UInstancedActorsData>* InstanceData = PerActorClassInstanceData.FindByPredicate([ActorClass, AdditionalInstanceTags](TObjectPtr<UInstancedActorsData> InstanceData)
		{ 
	...
	UInstancedActorsData* NewInstanceData = CreateNextInstanceActorData(ActorClass, AdditionalInstanceTags);
...
void AInstancedActorsManager::PreRegisterAllComponents()
{
	...
	for (UInstancedActorsData* InstanceData : PerActorClassInstanceData)
	{
		...
		for (TObjectPtr<UInstancedStaticMeshComponent> ISMComponent : InstanceData->EditorPreviewISMComponents)
		{
			...
			{
				ensureAlwaysMsgf(false, TEXT("UInstancedActorsData has a null EditorPreviewISMComponents element: %s"), *InstanceData->GetDebugName(/*bCompact*/ false));
				continue;
			}
...
FInstancedActorsInstanceHandle AInstancedActorsManager::AddActorInstance(TSubclassOf<AActor> ActorClass, FTransform InstanceTransform, bool bWorldSpace, const FInstancedActorsTagSet& AdditionalInstanceTags)
{
	UInstancedActorsData& InstanceData = GetOrCreateActorInstanceData(ActorClass, AdditionalInstanceTags);
			...
			InstancedActorLocationQuery.ForEachEntityChunkInCollections(EntityCollections, ExecutionContext, [&IterationContext, &InstanceHandle, &Operation, &bContinue](FMassExecutionContext& Context)
				{
					...
					for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
					{
						...
						const FInstancedActorsFragment& InstancedActorFragment = InstancedActorFragments[EntityIt];
...
template <typename TBoundsType>
bool AInstancedActorsManager::ForEachInstance(const TBoundsType& QueryBounds, FInstanceOperationFunc Operation, FInstancedActorsIterationContext& IterationContext, TOptional<FInstancedActorDataPredicateFunc> InstancedActorDataPredicate) const
{
	return ForEachInstance([&QueryBounds, &Operation](const FInstancedActorsInstanceHandle& InstanceHandle, const FTransform& InstanceTransform, FInstancedActorsIterationContext& IterationContext)
		{
			if (UE::InstancedActors::PassesBoundsTest(QueryBounds, UE::InstancedActors::EBoundsTestType::Intersect, InstanceHandle, InstanceTransform))
			{
...
void AInstancedActorsManager::SetUpEditorPreviewISMComponent(TNotNull<UInstancedStaticMeshComponent*> ISMComponent)
{
#if WITH_EDITORONLY_DATA
...
AActor* AInstancedActorsManager::FindOrCreateActor(const FActorInstanceHandle& Handle)
{
	...
	if (Actor == nullptr && EntityView.IsValid())
	{
#if WITH_INSTANCEDACTORS_DEBUG
		{
			...
			const int32 InstancedActorDataIndex = FInstancedActorsInstanceIndex::ExtractInstanceDataID(CompositeIndex);
			const FInstancedActorsInstanceIndex InternalInstanceIndex = FInstancedActorsInstanceIndex(FInstancedActorsInstanceIndex::ExtractInternalInstanceIndex(CompositeIndex));
			const UInstancedActorsData* InstanceData = PerActorClassInstanceData.IsValidIndex(InstancedActorDataIndex) ? PerActorClassInstanceData[InstancedActorDataIndex] : nullptr;
