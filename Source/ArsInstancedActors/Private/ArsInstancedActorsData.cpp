...
#define DO_COLLISION_INDEX_DEBUG (1 && WITH_ARSINSTANCEDACTORS_DEBUG)
...
DECLARE_DWORD_ACCUMULATOR_STAT(TEXT("Registered ISMs"), STAT_RegisteredISMs, STATGROUP_ArsInstancedActorsRendering);
DECLARE_DWORD_ACCUMULATOR_STAT(TEXT("Total Instance Count"), STAT_TotalInstanceCount, STATGROUP_ArsInstancedActorsRendering);
...
namespace UE::ArsInstancedActors
{
	namespace CVars
	{
		...
		FAutoConsoleVariableRef CVarCookCompressedInstances(
			...
			TEXT("Enables instances transform compression on cook for all AArsInstancedActorsManager's"),
			ECVF_Cheat);
		...
		FAutoConsoleVariableRef CVarForceSyncLoadVisualizations(
			...
			TEXT("If enabled, forces all calls to UArsInstancedActorsData::AddVisualizationAsync to immediately sync load required assets and ")
		...
		bool bCanHydrateLogicEnabled = true;
		FAutoConsoleVariableRef CVarCanHydrateLogicEnabled(
			TEXT("IA.EnableCanHydrateLogic"),
			bCanHydrateLogicEnabled,
			TEXT("Toggles special handling of instanced marked as non-hydrating via settings (MaxActorDistance ==0)"),
			ECVF_Default);
	} // CVars
	...
	FExemplarActorData::~FExemplarActorData()
	{
		if (CVars::bEnableReleasingEntityTemplatesAndExemplarActors)
		{
			...
			ArsInstancedActorsSubsystem->UnregisterExemplarActorClass(ExemplarActor->GetClass());
...
//-----------------------------------------------------------------------------
// UArsInstancedActorsData
//-----------------------------------------------------------------------------
void UArsInstancedActorsData::Initialize()
{
	AArsInstancedActorsManager& Manager = GetManagerChecked();

	// Get the settings setup nice and early.
	UArsInstancedActorsSubsystem& InstancedActorSubsystem = Manager.GetInstancedActorSubsystemChecked();
	SharedSettings = InstancedActorSubsystem.GetOrCompileSettingsForActorClass(ActorClass);
	const FArsInstancedActorsSettings* Settings = GetSettingsPtr<const FArsInstancedActorsSettings>();
	...
	if (Settings && Settings->bOverride_ActorClass && Settings->ActorClass)
	{
		...
		CachedLocalBounds = AArsInstancedActorsManager::CalculateBounds(ActorClass);
	...
	CombinedTags = AdditionalTags.GetTags();

	// Allow settings to combine its tags with AdditionalTags
	// @todo: resolve ambiguity with how the "overrides" should work with GameplayTags. The override flag is used both to solve inheritance-based
	// priority overriding of values, and to denote that a logical override will apply to IAD behavior. However, the end-result use case of
	// GameplayTags for IADs is to merge the settings-based tags with those that may have been baked into the IAD (i.e., AdditionalTags).
	// We may need to add a "merge" functionality to settings that would work for GameplayTags
	if (Settings && Settings->bOverride_GameplayTags)
	{
		CombinedTags.AppendTags(Settings->GameplayTags);
	}
	...
	//FArsInstancedActorsVisualizationDesc DefaultVisualiation = FArsInstancedActorsVisualizationDesc::FromActor(ExemplarActor, &UE::ArsInstancedActors::VisualizationDescrFromActorAdditionalSteps);
	FArsInstancedActorsVisualizationDesc DefaultVisualiation = InstancedActorSubsystem.CreateVisualDescriptionFromActor(*ExemplarActor);
	const uint8 VisualizationIndex = AddVisualization(DefaultVisualiation);
	check(VisualizationIndex == 0);

	bCanHydrate = (UE::ArsInstancedActors::CVars::bCanHydrateLogicEnabled == false) || (Settings == nullptr) || Settings->MaxActorDistance > 0;
...
void UArsInstancedActorsData::Deinitialize()
{
	if (bHasEverInitialized && UE::ArsInstancedActors::CVars::bEnableReleasingEntityTemplatesAndExemplarActors)
	{
	...
	CachedSetReplicatedActorRequests.Empty();
}

void UArsInstancedActorsData::CreateEntityTemplate(const AActor& ExemplarActor)
{
	...
	UMassStationaryDistanceVisualizationTrait* VisTrait = NewObject<UMassStationaryDistanceVisualizationTrait>(this, GET_ARSINSTANCEDACTORS_CONFIG_VALUE(GetStationaryVisualizationTraitClass()));
	if (UArsInstancedActorsVisualizationTrait* ArsInstancedActorsVisTrait = Cast<UArsInstancedActorsVisualizationTrait>(VisTrait))
	{
		ArsInstancedActorsVisTrait->InitializeFromInstanceData(*this);
	}
	EntityConfig.AddTrait(*VisTrait);

	// Allow UArsInstancedActorsComponent's to extend entity config
	ExemplarActor.ForEachComponent<UArsInstancedActorsComponent>(/*bIncludeFromChildActors*/ false
		, [this, &MassEntityManager](const UArsInstancedActorsComponent* InstancedActorComponent)
		{
	...
	// Allow UArsInstancedActorsComponent's to extend entity template
	ExemplarActor.ForEachComponent<UArsInstancedActorsComponent>(/*bIncludeFromChildActors*/ false
		, [this, &MassEntityManager, &ModifiedTemplate](const UArsInstancedActorsComponent* InstancedActorComponent)
		{
	...
	// @todo we should have GUID per IAD and use it here.
...
void UArsInstancedActorsData::ModifyEntityTemplate(FMassEntityTemplateData& ModifiedTemplate, const AActor&)
{
	// NOTE: Removes some tags that are added by default by the traits that we add to the original template,
	// but are toggled by the UArsInstancedActorsStationaryLODBatchProcessor and shouldn't be active when the Mass entities are created.
	ModifiedTemplate.GetMutableTags().Remove(UE::ArsInstancedActors::GetDetailedLODTags());
}

void UArsInstancedActorsData::ReleaseEntityTemplate()
{
	UWorld* World = GetWorld();
	check(World);
	...
	FMassEntityTemplateRegistry& TemplateRegistry = MassSpawnerSubsystem->GetMutableTemplateRegistryInstance();
	TemplateRegistry.DestroyTemplate(EntityTemplateID);
}

void UArsInstancedActorsData::SpawnEntities()
{
	...
	// Prepare slots for UArsInstancedActorsInitializerProcessor to place corresponding entity handles in.
	// Note: We can't simply use SpawnEntities returned array directly, as we only spawn entities
	//       to for `valid` InstanceTransforms. By letting UArsInstancedActorsInitializerProcessor store handles for
	...
	FArsInstancedActorsMassSpawnData SpawnData;
	SpawnData.InstanceData = this;

	// Spawn entities
	UE_LOG(LogArsInstancedActors, Verbose, TEXT("\t%s spawning %u entities"), *GetDebugName(/*bCompact*/ true), NumValidInstances);
	FConstStructView SpawnDataView = FConstStructView::Make(SpawnData);
	TArray<FMassEntityHandle> SpawnedEntities;
	MassSpawnerSubsystem->SpawnEntities(EntityTemplateID, NumValidInstances, SpawnDataView, UArsInstancedActorsInitializerProcessor::StaticClass(), SpawnedEntities);
	...
	if (UE::ArsInstancedActors::CVars::bUpdateNextTickTimeFragments)
	{
		FArsInstancedActorsDataSharedFragment* AsShared = SharedInstancedActorDataStruct.GetPtr<FArsInstancedActorsDataSharedFragment>();
		// our shared fragment was being ticked in the past, so it's possible it's scheduled to tick in quite some time
		// while we need the update ASAP. We're letting the UArsInstancedActorsSubsystem know to reschedule it.
		if (ensure(AsShared) && AsShared->LastTickTime > 0.0)
		{
			UArsInstancedActorsSubsystem* InstancedActorSubsystem = UE::ArsInstancedActors::Utils::GetArsInstancedActorsSubsystem(*World);
			check(InstancedActorSubsystem);
			InstancedActorSubsystem->UpdateAndResetTickTime(SharedInstancedActorDataStruct);
		}
	}

	// only once we spawned entities we can apply the "SetReplicatedActor" that came in before this InstancedActorData was ready to receive them.
	for (const FSetReplicatedActorRequests& Request : CachedSetReplicatedActorRequests)
	{
		if (AActor* Actor = Request.Actor.Get())
		{
			SetReplicatedActor(Request.InstanceIndex, *Actor);
		}
	}
	// we can afford to release the memory, since this array is only ever expected to be used at IAD's boot up
	CachedSetReplicatedActorRequests.Empty();
}

void UArsInstancedActorsData::DespawnEntities()
{
	UWorld* World = GetWorld();
	check(World);
	...
	UArsInstancedActorsRepresentationSubsystem* RepresentationSubsystem = World->GetSubsystem<UArsInstancedActorsRepresentationSubsystem>();
	...
	FMassEntityQuery InstancedActorLocationQuery(MassEntityManager.AsShared());
	...
	{
		InstancedActorLocationQuery.ForEachEntityChunk(Collection, ExecutionContext, [this, &WorldToLocalTranslation, &ManagerTransform](FMassExecutionContext& Context)
			{
				...
				{
					FArsInstancedActorsInstanceIndex InstanceIndex = InstancedActorFragments[EntityIt].InstanceIndex;
					InstanceTransforms[InstanceIndex.GetIndex()] = TransformsList[EntityIt].GetTransform();

					checkf(UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(InstanceTransforms[InstanceIndex.GetIndex()]), TEXT("Found Mass entity with unexpected Scale 0 'invalid' transform"));
	...
	checkSlow(NumValidInstances == Algo::CountIf(InstanceTransforms, [](const FTransform& InstanceTransform)
		{ return UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(InstanceTransform); }));
	...
	if (bApplyManagerTranslationOnly)
	{
		...
		{
			if (UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(InstanceTransform))
			{
	...
	FArsInstancedActorsDataSharedFragment ManagerFragment;
	ManagerFragment.InstanceData = this;
	FSharedStruct SharedFragmentInstance = MassEntityManager.GetOrCreateSharedFragment(ManagerFragment);

	FArsInstancedActorsDataSharedFragment* AsSharedManagerFragment = SharedFragmentInstance.GetPtr<FArsInstancedActorsDataSharedFragment>();
	if (ensure(AsSharedManagerFragment) && AsSharedManagerFragment->InstanceData == this)
	{
		...
		AsSharedManagerFragment->BulkLOD = EArsInstancedActorsBulkLOD::MAX;
	}
...
AArsInstancedActorsManager* UArsInstancedActorsData::GetManager() const
{
	if (UObject* Outer = GetOuter())
	{
		check(Outer->IsA<AArsInstancedActorsManager>());
		return static_cast<AArsInstancedActorsManager*>(Outer);
	}
	return nullptr;
}

AArsInstancedActorsManager& UArsInstancedActorsData::GetManagerChecked() const
{
	AArsInstancedActorsManager* Manager = GetManager();
	check(Manager);
	return *Manager;
}

UArsInstancedActorsData* UArsInstancedActorsData::GetInstanceDataForEntity(const FMassEntityManager& EntityManager, const FMassEntityHandle EntityHandle)
{
	FMassEntityView EntityView(EntityManager, EntityHandle);
	if (EntityView.IsSet())
	{
		if (FArsInstancedActorsFragment* InstancedActorFragment = EntityView.GetFragmentDataPtr<FArsInstancedActorsFragment>())
		{
...
bool UArsInstancedActorsData::HasSpawnedEntities() const
{
	return !Entities.IsEmpty();
}
int32 UArsInstancedActorsData::GetNumInstances() const
{
...
int32 UArsInstancedActorsData::GetNumFreeInstances() const
{
	return GetNumInstances() - NumValidInstances;
}

bool UArsInstancedActorsData::IsValidInstance(const FArsInstancedActorsInstanceHandle& InstanceHandle) const
{
	...
	{
		return InstanceHandle.InstancedActorData == this && InstanceTransforms.IsValidIndex(InstanceHandle.GetIndex()) && UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(InstanceTransforms[InstanceHandle.GetIndex()]);
	}
}

#if WITH_EDITOR
FArsInstancedActorsInstanceHandle UArsInstancedActorsData::AddInstance(const FTransform& Transform, const bool bWorldSpace)
{
	checkf(GetManagerChecked().HasActorBegunPlay() == false, TEXT("UArsInstancedActorsData doesn't yet support runtime addition of instances"));

	const bool bIsValidInstanceTransform = UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(Transform);
	...
	int32 NewInstanceIndex = UE::ArsInstancedActors::CVars::bRecycleInvalidInstances ? InstanceTransforms.IndexOfByPredicate([](const FTransform& InstanceTransform)
		{ return !UE::ArsInstancedActors::Helpers::IsValidInstanceTransform(InstanceTransform); })
		: INDEX_NONE;
	...
	return FArsInstancedActorsInstanceHandle(*this, FArsInstancedActorsInstanceIndex(NewInstanceIndex));
}

bool UArsInstancedActorsData::RemoveInstance(const FArsInstancedActorsInstanceHandle& InstanceToRemove)
{
	if (ensureMsgf(IsValidInstance(InstanceToRemove), TEXT("Attempting to remove invalid instance (%s) from %s"), *InstanceToRemove.GetDebugName(), *GetDebugName()))
	{
		...
		UE::ArsInstancedActors::Helpers::InvalidateInstanceTransform(InstanceTransforms[InstanceToRemove.GetIndex()]);
		--NumValidInstances;
...
bool UArsInstancedActorsData::SetInstanceTransform(const FArsInstancedActorsInstanceHandle& InstanceHandle, const FTransform& Transform, const bool bWorldSpace)
{
	checkf(GetManagerChecked().HasActorBegunPlay() == false, TEXT("UArsInstancedActorsData doesn't yet support runtime transformation of instances"));
...
void UArsInstancedActorsData::ForEachEditorPreviewISMC(TFunctionRef<bool(UInstancedStaticMeshComponent& /*ISMComponent*/)> InFunction) const
{
...
FArsInstancedActorsInstanceIndex UArsInstancedActorsData::GetInstanceIndexForEntity(const FMassEntityHandle EntityHandle) const
{
	FMassEntityManager& EntityManager = GetMassEntityManagerChecked();
	if (ensure(EntityManager.IsEntityValid(EntityHandle)))
	{
		const FArsInstancedActorsFragment* InstancedActorFragment = EntityManager.GetFragmentDataPtr<FArsInstancedActorsFragment>(EntityHandle);
	...
	return FArsInstancedActorsInstanceIndex();
}

FMassEntityHandle UArsInstancedActorsData::GetEntity(FArsInstancedActorsInstanceIndex InstanceIndex) const
{
...
void UArsInstancedActorsData::SetSharedInstancedActorDataStruct(FSharedStruct InSharedStruct)
{
	checkf(InSharedStruct.GetPtr<FArsInstancedActorsDataSharedFragment>(), TEXT("We expect only FArsInstancedActorsDataSharedFragment-base types here"));
	...
	SharedInstancedActorDataStruct = TStructView<FArsInstancedActorsDataSharedFragment>(InSharedStruct.GetMemory());
}

FString UArsInstancedActorsData::GetDebugName(bool bCompact) const
{
	if (bCompact)
	{
...
void UArsInstancedActorsData::PostDuplicate(EDuplicateMode::Type DuplicateMode)
{
...
void UArsInstancedActorsData::PostLoad()
{
	Super::PostLoad();
...
void UArsInstancedActorsData::SetupLoadedInstances()
{
	...
	if (!Bounds.IsValid)
	{
		...
		{
			// Note: UArsInstancedActorsData::Bounds mustn't be 0 sized by default, otherwise the first AddInstance
	...
	AdditionalTags.CalcHash();
}

void UArsInstancedActorsData::UpdateCullDistance()
{
	...
	for (FArsInstancedActorsVisualizationInfo& Visualization : InstanceVisualizations)
	{
		for (UInstancedStaticMeshComponent* ISMComponent : Visualization.ISMComponents)
		{
			...
			const int32 FarDistanceMaxTriangleCount = UE::ArsInstancedActors::CVars::LowLODMaxTriangleCountForFarDistanceRendering;
			...
			bool bScreenSizeDistanceCulled = LowLODTriangleCount > FarDistanceMaxTriangleCount || !UE::ArsInstancedActors::CVars::bEnableFarDistanceRendering;
			...
			float ISMMaxDrawDistance = /*bScreenSizeDistanceCulled ? DistanceCulledMaxDrawDistance : */UE::ArsInstancedActors::CVars::MaxDrawDistance;

			// Clamp to per instance max draw distance as well setup by the settings
			if (Settings && Settings->bOverride_MaxInstanceDistances)
			{
				ISMMaxDrawDistance = SettingsMaxInstanceDistance > 0 ? SettingsMaxInstanceDistance : UE::ArsInstancedActors::CVars::MaxDrawDistance;
			}
			...
			ISMMaxDrawDistance = UE::ArsInstancedActors::CVars::ForceMaxDrawDistance ? UE::ArsInstancedActors::CVars::MaxDrawDistance : ISMMaxDrawDistance;
			...
			ISMLowLODDrawDistance = FMath::Min(ISMLowLODDrawDistance, UE::ArsInstancedActors::CVars::ForcedLowLODStartDrawDistance);
...
FMassEntityManager& UArsInstancedActorsData::GetMassEntityManagerChecked() const
{
	return GetManagerChecked().GetMassEntityManagerChecked();
}

void UArsInstancedActorsData::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(UArsInstancedActorsData, InstanceDeltas, RepParams);
}

#if UE_WITH_IRIS
void UArsInstancedActorsData::RegisterReplicationFragments(UE::Net::FFragmentRegistrationContext& Context, UE::Net::EFragmentRegistrationFlags RegistrationFlags)
{
	UE::Net::FReplicationFragmentUtil::CreateAndRegisterFragmentsForObject(this, Context, RegistrationFlags);
}
#endif

void UArsInstancedActorsData::SetInstanceCurrentLifecyclePhase(FArsInstancedActorsInstanceIndex InstanceIndex, uint8 InCurrentLifecyclePhaseIndex)
{
	AArsInstancedActorsManager& Manager = GetManagerChecked();
...
void UArsInstancedActorsData::RemoveInstanceLifecyclePhaseDelta(FArsInstancedActorsInstanceIndex InstanceIndex)
{
	AArsInstancedActorsManager& Manager = GetManagerChecked();
	check(Manager.HasAuthority());
...
void UArsInstancedActorsData::RemoveInstanceLifecyclePhaseTimeElapsedDelta(FArsInstancedActorsInstanceIndex InstanceIndex)
{
	AArsInstancedActorsManager& Manager = GetManagerChecked();
	check(Manager.HasAuthority());
...
void UArsInstancedActorsData::ApplyInstanceDeltas()
{
	const TArray<FArsInstancedActorsDelta>& Deltas = InstanceDeltas.GetInstanceDeltas();

#if WITH_ARSINSTANCEDACTORS_DEBUG
	UE_LOG(LogArsInstancedActors, Verbose, TEXT("Applying %d instance deltas (D: %u, L: %u, LT: %u) to %s"), Deltas.Num(), InstanceDeltas.GetNumDestroyedInstanceDeltas(), InstanceDeltas.GetNumLifecyclePhaseDeltas(), InstanceDeltas.GetNumLifecyclePhaseTimeElapsedDeltas(), *GetDebugName());
#endif
	...
	if (!HasSpawnedEntities())
	{
		...
		// apply these later in AArsInstancedActorsManager::InitializeModifyAndSpawnEntities
		return;
	}

	FMassEntityManager& EntityManager = GetMassEntityManagerChecked();

	TArray<FArsInstancedActorsInstanceIndex> EntitiesToRemove;

	for (const FArsInstancedActorsDelta& Delta : Deltas)
	{
...
void UArsInstancedActorsData::ApplyInstanceDeltas(TConstArrayView<int32> InstanceDeltaIndices)
{
#if WITH_ARSINSTANCEDACTORS_DEBUG
	UE_LOG(LogArsInstancedActors, Verbose, TEXT("Applying %d instance deltas to %s"), InstanceDeltaIndices.Num(), *GetDebugName());
#endif
...
void UArsInstancedActorsData::RollbackInstanceDeltas(TConstArrayView<int32> InstanceDeltaIndices)
{
#if WITH_ARSINSTANCEDACTORS_DEBUG
	UE_LOG(LogArsInstancedActors, Verbose, TEXT("Rolling back %d instance deltas on %s"), InstanceDeltaIndices.Num(), *GetDebugName());
#endif
	...
	{
		const FArsInstancedActorsDelta& Delta = Deltas[InstanceDeltaIndex];

		RollbackInstanceDelta(EntityManager, Delta);
	}
}

void UArsInstancedActorsData::ApplyInstanceDelta(FMassEntityManager& EntityManager, const FArsInstancedActorsDelta& InstanceDelta, TArray<FArsInstancedActorsInstanceIndex>& OutEntitiesToRemove)
{
	...
	if (EntityManager.IsEntityValid(Entity))
	{
		...
		{
			OutEntitiesToRemove.Add(FArsInstancedActorsInstanceIndex(InstanceIndex));
		}
	}
}

void UArsInstancedActorsData::RollbackInstanceDelta(FMassEntityManager& EntityManager, const FArsInstancedActorsDelta& InstanceDelta)
{
	// mz@todo IA: move this section back
...
void UArsInstancedActorsData::RuntimeRemoveInstances(TConstArrayView<FArsInstancedActorsInstanceIndex> InstancesToRemove)
{
	...
	if (HasSpawnedEntities())
	{
		...
		for (FArsInstancedActorsInstanceIndex InstanceToRemove : InstancesToRemove)
		{
...
void UArsInstancedActorsData::RuntimeRemoveAllInstances()
{
	checkSlow(GetManagerChecked().GetWorld()->IsGameWorld());
	...
	else
	{
		for (FTransform& InstanceTransform : InstanceTransforms)
		{
			UE::ArsInstancedActors::Helpers::InvalidateInstanceTransform(InstanceTransform);
		}
		NumValidInstances = 0;
	}
...
void UArsInstancedActorsData::EjectInstanceActor(FArsInstancedActorsInstanceIndex InstanceToEject, AActor& ActorToEject)
{
	UE_LOG(LogArsInstancedActors, Verbose, TEXT("Ejecting instanced actor: %s"), *ActorToEject.GetPathName());
...
void UArsInstancedActorsData::SetReplicatedActor(FArsInstancedActorsInstanceIndex Instance, AActor& ReplicatedActor)
{
	checkf(!ReplicatedActor.HasAuthority(), TEXT("SetReplicatedActor must only be called on clients where we explictly connect actor to entities via UArsInstancedActorsComponent::OnRep_InstanceHandle"));

	if (HasSpawnedEntities() == false)
	{	
		// we need to cache the request to apply it once we have spawned the entities
		CachedSetReplicatedActorRequests.Emplace(Instance, &ReplicatedActor);
		return;
	}
	...
	if (UArsInstancedActorsSubsystem* InstancedActorSubsystem = GetManagerChecked().GetInstancedActorSubsystem())
	{
		FArsInstancedActorsInstanceHandle InstanceHandle(*this, Instance);
		InstancedActorSubsystem->MarkInstanceRepresentationDirty(InstanceHandle);
	}
}

void UArsInstancedActorsData::ClearReplicatedActor(FArsInstancedActorsInstanceIndex Instance, AActor& ExpectedActor)
{
	checkf(!GetManagerChecked().HasAuthority(), TEXT("ClearReplicatedActor must only be called on clients where we explictly disconnect actors from entities via UArsInstancedActorsComponent::EndPlay"));
...
AActor* UArsInstancedActorsData::UnlinkActor(FArsInstancedActorsInstanceIndex InstanceToUnlink)
{
...
uint8 UArsInstancedActorsData::AllocateVisualization()
{
	...
	if (AllocatedVisualizationIndex == INDEX_NONE)
	{
		...
		checkf(AllocatedVisualizationIndex < TNumericLimits<uint8>::Max(), TEXT("No more than UINT8_MAX - 1 InstanceVisualizations can be defined for a single UArsInstancedActorsData as uint8 is used to index InstanceVisualizations and UINT8_MAX is reserved for invalid index"));
	}
...
void UArsInstancedActorsData::InitializeVisualization(uint8 AllocatedVisualizationIndex, const FArsInstancedActorsVisualizationDesc& VisualizationDesc)
{
	check(InstanceVisualizations.IsValidIndex(AllocatedVisualizationIndex));
	check(InstanceVisualizationAllocationFlags.IsValidIndex(AllocatedVisualizationIndex));
	check(InstanceVisualizationAllocationFlags[AllocatedVisualizationIndex] == true);
	FArsInstancedActorsVisualizationInfo& NewVisualization = InstanceVisualizations[AllocatedVisualizationIndex];
...
uint8 UArsInstancedActorsData::AddVisualization(FArsInstancedActorsVisualizationDesc& InOutVisualizationDesc)
{
	// Reuse free or create new InstanceVisualizations entry
	uint8 NewVisualizationIndex = AllocateVisualization();

	if (ExemplarActorData.IsValid() && ExemplarActorData->Actor)
	{
		GetManagerChecked().GetInstancedActorSubsystemChecked().ModifyVisualDescriptionForActor(TNotNull<AActor*>(ExemplarActorData->Actor.Get()), InOutVisualizationDesc);
	}

	// Init new visualization
	InitializeVisualization(NewVisualizationIndex, InOutVisualizationDesc);

	return NewVisualizationIndex;
}

uint8 UArsInstancedActorsData::AddVisualizationAsync(const FArsInstancedActorsSoftVisualizationDesc& SoftVisualizationDesc)
{
	...
	if (UE::ArsInstancedActors::CVars::bForceSyncLoadVisualizations || VisualizationAssetsToLoad.IsEmpty())
	{
		// Nothing needing to load. Resolve hard visualization description and add immediately.
		FArsInstancedActorsVisualizationDesc VisualizationDesc(SoftVisualizationDesc);
	...
	else
	{
		...
		FArsInstancedActorsVisualizationInfo& ReservedVisualization = InstanceVisualizations[ReservedVisualizationIndex];

		TWeakObjectPtr<UArsInstancedActorsSubsystem> WeakArsInstancedActorsSubsystem = GetManagerChecked().GetInstancedActorSubsystem();

		// Async load assets and complete initialization when load completes
		ReservedVisualization.AssetLoadHandle = UAssetManager::Get().GetStreamableManager().RequestAsyncLoad(VisualizationAssetsToLoad, FStreamableDelegate::CreateWeakLambda(this, [this, SoftVisualizationDesc, ReservedVisualizationIndex, WeakArsInstancedActorsSubsystem]()
			{
				...
				FArsInstancedActorsVisualizationInfo& Visualization = InstanceVisualizations[ReservedVisualizationIndex];

				// Resolve hard visualization description
				FArsInstancedActorsVisualizationDesc VisualizationDesc(SoftVisualizationDesc);

				if (WeakArsInstancedActorsSubsystem.IsValid() && ExemplarActorData.IsValid() && ExemplarActorData->Actor)
				{
					WeakArsInstancedActorsSubsystem->ModifyVisualDescriptionForActor(TNotNull<AActor*>(ExemplarActorData->Actor.Get()), VisualizationDesc);
				}
...
void UArsInstancedActorsData::ForEachVisualization(TFunctionRef<bool(uint8 /*VisualizationIndex*/, const FArsInstancedActorsVisualizationInfo& /*Visualization*/)> InFunction, const bool bSkipAsyncLoadingVisualizations) const
{
	...
	for (TConstSetBitIterator<> AllocatedVisualizationsIt(InstanceVisualizationAllocationFlags); AllocatedVisualizationsIt; ++AllocatedVisualizationsIt)
	{
		...
		const FArsInstancedActorsVisualizationInfo& Visualization = InstanceVisualizations[VisualizationIndex];
...
void UArsInstancedActorsData::SwitchInstanceVisualization(FArsInstancedActorsInstanceIndex InstanceToSwitch, uint8 NewVisualizationIndex)
{
	...
	const FArsInstancedActorsVisualizationInfo& NewVisualization = InstanceVisualizations[NewVisualizationIndex];

	FArsInstancedActorsMeshSwitchFragment MeshSwitchFragment;
...
void UArsInstancedActorsData::RemoveVisualization(uint8 VisualizationIndex)
{
	...
	FArsInstancedActorsVisualizationInfo& RemovedVisualization = InstanceVisualizations[VisualizationIndex];
	...
	RemovedVisualization = FArsInstancedActorsVisualizationInfo();
	check(InstanceVisualizationAllocationFlags.IsValidIndex(VisualizationIndex));
	InstanceVisualizationAllocationFlags[VisualizationIndex] = false;
}

void UArsInstancedActorsData::RemoveAllVisualizations()
{
...
void UArsInstancedActorsData::DestroyInstance(FArsInstancedActorsInstanceIndex InstanceToDestroy)
{
	AArsInstancedActorsManager& Manager = GetManagerChecked();
...
void UArsInstancedActorsData::OnInstancedActorDestroyed(AActor& DestroyedActor, const FMassEntityHandle EntityHandle)
{
	...
	FArsInstancedActorsInstanceIndex DestroyedInstanceIndex = GetInstanceIndexForEntity(EntityHandle);
	if (ensureMsgf(DestroyedInstanceIndex.IsValid(), TEXT("Somehow OnInstancedActorDestroyed was received for an actor which we assume was spawned by ArsInstancedActors but %s isn't tracking this entity"), *GetDebugName()))
	{
...
bool UArsInstancedActorsData::OnInstancedActorMoved(AActor& MovedActor, const FMassEntityHandle EntityHandle)
{
	const FArsInstancedActorsSettings& Settings = GetSettings<const FArsInstancedActorsSettings>();
	...
	if (Settings.bEjectOnActorMoved)
	{
		...
		if (ensure(EntityManager.IsEntityValid(EntityHandle)))
		{
			...
			if (ensure(TransformFragment != nullptr))
			{
				...
				{
					const FArsInstancedActorsFragment* InstancedActorFragment = EntityView.GetFragmentDataPtr<FArsInstancedActorsFragment>();
					if (ensure(InstancedActorFragment != nullptr))
					{
...
void UArsInstancedActorsData::OnPersistentDataRestored()
{
	// AArsInstancedActorsManager::SerializeInstancePersistenceData will have restored the InstanceDeltas list and replicated that
	// for application on clients in OnRep_InstanceDeltas, here we then apply the restored delta changes on the server.
	ApplyInstanceDeltas();
}

void UArsInstancedActorsData::OnRep_InstanceDeltas(TConstArrayView<int32> UpdatedInstanceDeltaIndices)
{
	ApplyInstanceDeltas(UpdatedInstanceDeltaIndices);
}

void UArsInstancedActorsData::OnRep_PreRemoveInstanceDeltas(TConstArrayView<int32> RemovedInstanceDeltaIndices)
{
	RollbackInstanceDeltas(RemovedInstanceDeltaIndices);
}

int32 UArsInstancedActorsData::GetEntityIndexFromCollisionIndex(const UInstancedStaticMeshComponent& ISMComponent, const int32 CollisionIndex) const
{
	const UArsInstancedActorsRepresentationSubsystem* RepresentationSubsystem = UWorld::GetSubsystem<UArsInstancedActorsRepresentationSubsystem>(GetWorld());
	...
	VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("%s collision index to entity, looking for %d"), *GetNameSafe(ActorClass), CollisionIndex);
#endif // DO_COLLISION_INDEX_DEBUG

	TArray<int32>* RelevantIdMap = nullptr;

	for (const FArsInstancedActorsVisualizationInfo& IAVisualization : InstanceVisualizations)
	{
		...
		{
			// We can't use GetISMCSharedDataForDescriptionIndex() with the MassStaticMeshDescHandle, since it corresponds to a given FStaticMeshInstanceVisualizationDesc, not its owned ISMC data
			const FMassISMCSharedData* RelevantISMCData = RepresentationSubsystem->GetISMCSharedDataForInstancedStaticMesh(&ISMComponent);
			...
			if (RelevantISMCData && (RelevantISMCData->GetComponentInstanceIdTouchCounter() != IAVisualization.CachedTouchCounter))
			{
...
#if DO_COLLISION_INDEX_DEBUG
				VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\tRecaching map. Stored touch %d, current %d"), IAVisualization.CachedTouchCounter, RelevantISMCData->GetComponentInstanceIdTouchCounter());
				VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\t(initial) Valid entities: %d; ISM collision map size: %d"), NumValidInstances, IdMap.Num());
				CVLOG_COLLISIONINDEX(ISMComponent.GetNumInstances() != IdMap.Num(), this, LogArsInstancedActors, Error, TEXT("\t\tMismatch with num ISM instances: %d"), ISMComponent.GetNumInstances());

				VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\tISM map:"));
				for (auto Pair : IdMap)
				{
					VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\t\t%s -> %d"), *Pair.Key.DebugGetDescription(), ISMComponent.GetInstanceIndexForId(Pair.Value));
				}
#endif // DO_COLLISION_INDEX_DEBUG
				...
				VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\tResulting mapping:"));
				for (int32 EntityIndex = 0; EntityIndex < Entities.Num(); ++EntityIndex)
				{
					if (Entities[EntityIndex].IsValid() == false)
					{
						VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\t\t%d: --- "), EntityIndex);
						continue;
					}
					...
					{
						VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("\t\t\t%d: %s collision index: %d"), EntityIndex, *EntityHandle.DebugGetDescription(), ISMComponent.GetInstanceIndexForId(*PrimitiveInstanceId));
					...
					else
					{
						if (const bool bIsValidEntity = EntityManager.IsEntityValid(EntityHandle))
						{
							...
							{
								VLOG_COLLISIONINDEX(this, LogArsInstancedActors, VeryVerbose, TEXT("\t\t\t%d: %s collision index: none, is an ACTOR"), EntityIndex, *EntityHandle.DebugGetDescription());
							}
							else
							{
								VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Error, TEXT("\t\t\t%d: %s collision index: NOT FOUND!"), EntityIndex, *EntityHandle.DebugGetDescription());
							}
						}
						else
						{
							VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Warning, TEXT("\t\t\t%d: %s NO collision index - STALE handle"), EntityIndex, *EntityHandle.DebugGetDescription());
						}
					}
#endif // DO_COLLISION_INDEX_DEBUG
				}
	...
	VLOG_COLLISIONINDEX(this, LogArsInstancedActors, Verbose, TEXT("Found EntityIndex  %d"), EntityIndex);
