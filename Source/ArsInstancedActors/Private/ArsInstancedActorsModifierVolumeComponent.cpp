...
namespace UE::ArsInstancedActors
{
	namespace Helpers
	{
		FTopLevelAssetPath FindLevelWorldPartitionAssetPath(const ULevel* Level)
		{
			FTopLevelAssetPath AssetPath;
			if (const UWorldPartitionRuntimeLevelStreamingCell* WorldPartitionCell = Cast<UWorldPartitionRuntimeLevelStreamingCell>(Level->GetWorldPartitionRuntimeCell()))
			{
				if (const UWorldPartitionLevelStreamingDynamic* WorldPartitionLevelStreaming = Cast<UWorldPartitionLevelStreamingDynamic>(WorldPartitionCell->GetLevelStreaming()))
				{
					AssetPath = WorldPartitionLevelStreaming->GetOuterWorldPartition().GetAssetPath();
				}
			}
...
//-----------------------------------------------------------------------------
// UArsInstancedActorsModifierVolumeComponent
//-----------------------------------------------------------------------------
UArsInstancedActorsModifierVolumeComponent::UArsInstancedActorsModifierVolumeComponent(const FObjectInitializer& ObjectInitializer)
...
void UArsInstancedActorsModifierVolumeComponent::BeginPlay()
{
	Super::BeginPlay();

	TRACE_CPUPROFILER_EVENT_SCOPE_STR("UArsInstancedActorsModifierVolumeComponent BeginPlay");
	...
	UArsInstancedActorsSubsystem* PreinitializedInstancedActorSubsystem = UArsInstancedActorsSubsystem::Get(this);
...
void UArsInstancedActorsModifierVolumeComponent::OnAddedToSubsystem(UArsInstancedActorsSubsystem& InstancedActorSubsystem, FArsInstancedActorsModifierVolumeHandle InModifierVolumeHandle)
{
	...
	if (BoundingBox.IsValid && !BoundingBox.GetSize().IsNearlyZero()) 
	{
		...
		UE::ArsInstancedActors::Debug::DebugDrawModifierVolumeBounds(UE::ArsInstancedActors::Debug::CVars::DebugModifiers, *this, FColorList::LightBlue.WithAlpha(30));
#endif // WITH_ARSINSTANCEDACTORS_DEBUG

		InstancedActorSubsystem.ForEachManager(BoundingBox, [this](AArsInstancedActorsManager& Manager)
		{
			// Call AddModifierVolume which calls UArsInstancedActorsModifierVolumeComponent::OnAddedToManager
...
void UArsInstancedActorsModifierVolumeComponent::EndPlay(EEndPlayReason::Type Reason)
{
	Super::EndPlay(Reason);

	TRACE_CPUPROFILER_EVENT_SCOPE_STR("UArsInstancedActorsModifierVolumeComponent EndPlay");

	UArsInstancedActorsSubsystem* InstancedActorSubsystem = UArsInstancedActorsSubsystem::Get(this);
	if (ensure(InstancedActorSubsystem))
	{
		...
		TArray<TWeakObjectPtr<AArsInstancedActorsManager>> ManagersToDeregisterFrom = ModifiedManagers;
		for (TWeakObjectPtr<AArsInstancedActorsManager>& ModifiedManager : ManagersToDeregisterFrom)
		{
...
void UArsInstancedActorsModifierVolumeComponent::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	
	DOREPLIFETIME_CONDITION(UArsInstancedActorsModifierVolumeComponent, Radius, COND_InitialOnly);
	DOREPLIFETIME_CONDITION(UArsInstancedActorsModifierVolumeComponent, Extent, COND_InitialOnly);
	DOREPLIFETIME_CONDITION(UArsInstancedActorsModifierVolumeComponent, Shape, COND_InitialOnly);
}

void UArsInstancedActorsModifierVolumeComponent::OnAddedToManager(AArsInstancedActorsManager& Manager)
{
...
#if WITH_ARSINSTANCEDACTORS_DEBUG
	UE::ArsInstancedActors::Debug::DebugDrawModifierVolumeAddedToManager(UE::ArsInstancedActors::Debug::CVars::DebugModifiers, Manager, *this);
#endif // WITH_ARSINSTANCEDACTORS_DEBUG
}

void UArsInstancedActorsModifierVolumeComponent::OnRemovedFromManager(AArsInstancedActorsManager& Manager)
{
	check(ModifiedManagers.Contains(&Manager));
...
bool UArsInstancedActorsModifierVolumeComponent::TryRunPendingModifiers(AArsInstancedActorsManager& Manager, TBitArray<>& InOutPendingModifiers)
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("UArsInstancedActorsModifierVolumeComponent ModifyInstances");
	...
	check(GetOwner());
	if (bIgnoreOwnLevelsInstances)
	{
		// If logic matches, return true as if we have successfully run all modifiers, so they don't attempt to run again.
		ULevel* ManagerLevel = Manager.GetLevel();
		ULevel* OwnerLevel = GetOwner()->GetLevel();

		// If Actors are world partitioned, the ULevel pointers will not match even if they originally came from the same level.
		// Instead, compare the FTopLevelAssetPath from their World Partition streaming cells if applicable.
		// For non-world partitioned Actors, the logic will fall to the next block to do the original ULevel pointer comparison,
		// since either ManagerLoadedPath or OwnerLoadedPath will remain !IsValid()
		if (ManagerLevel != nullptr && OwnerLevel != nullptr)
		{
			FTopLevelAssetPath ManagerLoadedPath = UE::ArsInstancedActors::Helpers::FindLevelWorldPartitionAssetPath(ManagerLevel);
			FTopLevelAssetPath OwnerLoadedPath = UE::ArsInstancedActors::Helpers::FindLevelWorldPartitionAssetPath(OwnerLevel);

			if (ManagerLoadedPath.IsValid() && OwnerLoadedPath.IsValid() && ManagerLoadedPath == OwnerLoadedPath)
			{
				return true;
			}
		}

		if (ManagerLevel == OwnerLevel)
		{
			return true;
		}
	}
	...
	for (TBitArray<>::FIterator PendingModifierIt(InOutPendingModifiers); PendingModifierIt; ++PendingModifierIt)
	{
		if (PendingModifierIt)
		{
			...
			{
				UArsInstancedActorsModifierBase* Modifier = Modifiers[ModifierIndex];
...
FBoxSphereBounds UArsInstancedActorsModifierVolumeComponent::CalcBounds(const FTransform& LocalToWorld) const
{
...
FPrimitiveSceneProxy* UArsInstancedActorsModifierVolumeComponent::CreateSceneProxy()
{
	class FArsInstancedActorsModifierVolumeComponent final : public FPrimitiveSceneProxy
	{
	public:
		...
		FArsInstancedActorsModifierVolumeComponent(const UArsInstancedActorsModifierVolumeComponent* InComponent)
	...
	private:
		const EArsInstancedActorsVolumeShape Shape;
	...
	return new FArsInstancedActorsModifierVolumeComponent( this );
}
...
URemoveInstancesModifierVolumeComponent::URemoveInstancesModifierVolumeComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	URemoveArsInstancedActorsModifier* RemoveArsInstancedActorsModifier = CreateDefaultSubobject<URemoveArsInstancedActorsModifier>(TEXT("RemoveArsInstancedActorsModifier"));
	Modifiers.Add(RemoveArsInstancedActorsModifier);
}
