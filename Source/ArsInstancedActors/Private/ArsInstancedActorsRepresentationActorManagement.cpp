...
EMassActorSpawnRequestAction UArsInstancedActorsRepresentationActorManagement::OnPostActorSpawn(const FMassActorSpawnRequestHandle& SpawnRequestHandle
	, FConstStructView SpawnRequest, TSharedRef<FMassEntityManager> EntityManager) const
{
	...
	if (ResultAction == EMassActorSpawnRequestAction::Keep)
	{
		...
		if (AActor* Actor = Cast<AActor>(MassActorSpawnRequest.SpawnedActor))
		{
			...
			{
				RootComponent->TransformUpdated.AddUObject(this, &UArsInstancedActorsRepresentationActorManagement::OnSpawnedActorMoved, MassActorSpawnRequest.MassAgent);
			}
		}

		// Allow settings to turn off damage for this actor.
		UArsInstancedActorsSubsystem& InstancedActorSubsystem = UArsInstancedActorsSubsystem::GetChecked(MassActorSpawnRequest.SpawnedActor);
		...
		const FArsInstancedActorsSettings* Settings = SharedSettings.GetPtr<FArsInstancedActorsSettings>();
...
void UArsInstancedActorsRepresentationActorManagement::SetActorEnabled(const EMassActorEnabledType EnabledType, AActor& Actor, const int32 EntityIdx, FMassCommandBuffer& CommandBuffer) const
{
...
void UArsInstancedActorsRepresentationActorManagement::TeleportActor(const FTransform& Transform, AActor& Actor, FMassCommandBuffer& CommandBuffer) const
{
...
void UArsInstancedActorsRepresentationActorManagement::OnActorUnlinked(AActor& Actor)
{
...
void UArsInstancedActorsRepresentationActorManagement::OnSpawnedActorDestroyed(AActor& DestroyedActor, FMassEntityHandle EntityHandle) const
{
	...
	if (ensure(World))
	{
		...
		if (ensure(EntitySubsystem))
		{
			...
			{
				UArsInstancedActorsData* InstanceData = UArsInstancedActorsData::GetInstanceDataForEntity(EntityManager, EntityHandle);
...
void UArsInstancedActorsRepresentationActorManagement::OnSpawnedActorMoved(USceneComponent* MovedActorRootComponent, EUpdateTransformFlags TransformUpdateFlags, ETeleportType TeleportType, FMassEntityHandle EntityHandle) const
{
...
AActor* UArsInstancedActorsRepresentationActorManagement::FindOrInstantlySpawnActor(UMassRepresentationSubsystem& RepresentationSubsystem, FMassEntityManager& EntityManager, FMassEntityView& EntityView)
{
	...
	if (SpawnedActor)
	{
		...
		FMassTagBitSet TagsToRemove = UE::Mass::Utils::ConstructTagBitSet<EMassCommandCheckTime::CompileTimeCheck, FMassStationaryISMSwitcherProcessorTag, FArsInstancedActorsVisualizationProcessorTag>();
