...
// Helper macro for FArsInstancedActorsSettings::ToString
//
 ...
 * 
 * Named settings are FArsInstancedActorsSettings registered by name in UArsInstancedActorsProjectSettings::NamedSettingsRegistryType 
 * data registry.
 *
 * Class-specific settings are FArsInstancedActorsSettings registered by actor class via FArsInstancedActorsClassSettingsBase's in the 
 * UArsInstancedActorsProjectSettings::ActorClassSettingsRegistryType data registry. Named settings can be used as bases here via
 * FArsInstancedActorsClassSettingsBase::BaseSettings.
 *
 * @see UArsInstancedActorsProjectSettings for settings registry info.
 */
USTRUCT(BlueprintType)
struct FArsInstancedActorsSettings : public FTableRowBase
{
	GENERATED_BODY()

	// Bitflag per setting to choose if it should override a base setting

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta=(InlineEditConditionToggle), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta=(InlineEditConditionToggle), Category=ArsInstancedActors)
	uint8 bOverride_bModifierVolumeCheckFullyEnclosed : 1 = false;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta=(InlineEditConditionToggle), Category=ArsInstancedActors)
	uint8 bOverride_bControlPhysicsState : 1 = false;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta=(InlineEditConditionToggle), Category=ArsInstancedActors)
	uint8 bOverride_GameplayTags : 1 = false;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bInstancesCastShadows"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_MaxActorDistance"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bDisableAutoDistanceCulling"), Category=ArsInstancedActors)
	bool bDisableAutoDistanceCulling = false;

	/** If false, collision will not be managed by the mass LODs for this instance **/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bControlPhysicsState"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, EditFixedSize, Meta = (EditFixedOrder, EditCondition = "bOverride_MaxInstanceDistances"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, EditFixedSize, Meta = (EditFixedOrder, EditCondition = "bOverride_LODDistanceScales"), Category=ArsInstancedActors)
	TArray<float> LODDistanceScales = { 1.0f, 1.0f, 1.0f, 1.0f };

	/** If enabled, then asset will be used in distance field lighting */
	UPROPERTY(EditAnywhere, EditFixedSize, Meta = (EditFixedOrder, EditCondition = "bOverride_AffectDistanceFieldLighting"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_DetailedRepresentationLODDistance"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_ForceLowRepresentationLODDistance"), Category=ArsInstancedActors)
	double ForceLowRepresentationLODDistance = 27500.0;

	/** Distance in cm from which WPO is disabled */
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_WorldPositionOffsetDisableDistance"), Category=ArsInstancedActors)
	 ...
	 */
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_bEjectOnActorMoved"), Category=ArsInstancedActors)
	 ...
	 * as destroyed and transferred to persistence system.
	 */
	UPROPERTY(EditAnywhere, Meta = (EditCondition = "bOverride_ActorEjectionMovementThreshold"), Category=ArsInstancedActors)
	float ActorEjectionMovementThreshold = 1.0f;

	/** Can this object ever affect the navigation graph generation for AI etc. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bCanEverAffectNavigation"), Category=ArsInstancedActors)
	bool bCanEverAffectNavigation = true;

	/** What world partition grid should this instance be placed into. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_OverrideWorldPartitionGrid"), Category=ArsInstancedActors)
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_ScaleEntityCount", ClampMin="0.0", ClampMax="1.0", UIMin="0.8", UIMax="1.0"), Category=ArsInstancedActors)
	float ScaleEntityCount = 1.0f;

	/** Wholesale replace this type of entity with this actor instead. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_ActorClass"), Category=ArsInstancedActors)
	TSubclassOf<AActor> ActorClass;

	/** Turn on or off damage. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bCanBeDamaged"), Category=ArsInstancedActors)
	bool bCanBeDamaged = true;

	/** Completely disable modifier volumes for this type of instance. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bIgnoreModifierVolumes"), Category=ArsInstancedActors)
	bool bIgnoreModifierVolumes = false;

	/** By default, modifier volumes will do a bounds intersection test against instances. If this is true, modifiers will instead do a fully-enclosed bounds test */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_bModifierVolumeCheckFullyEnclosed"), Category=ArsInstancedActors)
	bool bModifierVolumeCheckFullyEnclosed = false;
	
	/** Gameplay Tags that can be used in Modifier queries */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Meta = (EditCondition = "bOverride_GameplayTags"), Category=ArsInstancedActors)
...
#if WITH_EDITORONLY_DATA
template<>
struct TStructOpsTypeTraits<FArsInstancedActorsSettings> : public TStructOpsTypeTraitsBase2<FArsInstancedActorsSettings>
{
	enum
	{
 ...
 * 
 * Per-class settings defined in UArsInstancedActorsProjectSettings::ActorClassSettingsRegistryType are automatically
 ...
 * 
 *    1) Default construted FArsInstancedActorsSettings
 *    2) UArsInstancedActorsProjectSettings::DefaultBaseSettingsName
 * 
 *    3) ActorClass::Super's FArsInstancedActorsClassSettingsBase BaseSettings (if any)
 *         [0]
 *         [1]
 *         [2]
 *         ...
 ...
 *
 *    7) UArsInstancedActorsProjectSettings::EnforcedSettingsName
 * 
 * @see FArsInstancedActorsSettings
 */
USTRUCT()
struct FArsInstancedActorsClassSettingsBase : public FTableRowBase
{
	GENERATED_BODY()
	 ...
	 * Note: UArsInstancedActorsProjectSettings::DefaultBaseSettingsName if specified is effectively inserted into this list at 0
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = ArsInstancedActors)
	TArray<FName> BaseSettings;

	/** A virtual function to be overridden by child-types that provide an instance of their specific FArsInstancedActorsSettings flavor */
	virtual FInstancedStruct MakeOverrideSettings() const { return FInstancedStruct::Make<FArsInstancedActorsSettings>(); }

};

/** 
 * Generic implementation of FArsInstancedActorsClassSettingsBase that's using the generic FArsInstancedActorsSettings to override
 * existing settings. If you want to extend the per-actor-class settings for your project then inhering either from 
 * FArsInstancedActorsClassSettings and add extra logic and properties to the child type, or inherit from FArsInstancedActorsClassSettingsBase
 * to provide a project-specific settings struct that inherits FArsInstancedActorsSettings.
 */
USTRUCT()
struct FArsInstancedActorsClassSettings : public FArsInstancedActorsClassSettingsBase
{
	GENERATED_BODY()

	/**
	 * Settings specific to ActorClass instances, applied after / overriding BaseSettings.
	 * Note: UArsInstancedActorsProjectSettings::EnforcedSettingsName if specified can still override these.
	 */ 
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = ArsInstancedActors)
	FArsInstancedActorsSettings OverrideSettings;
