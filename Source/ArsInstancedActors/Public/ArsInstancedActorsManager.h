...
namespace UE::ArsInstancedActors
{
	...
	bool PassesBoundsTest(const TBoundsType& QueryBounds, EBoundsTestType BoundsTestType, const FArsInstancedActorsInstanceHandle& InstanceHandle, const FTransform& InstanceTransform);

} // UE::ArsInstancedActors

DECLARE_STATS_GROUP(TEXT("InstanceActor Rendering"), STATGROUP_ArsInstancedActorsRendering, STATCAT_Advanced);

// @todo consider renaming to AStaticArsInstancedActorsManager. Will require renaming of the instance data type as well along
 ...
 * spawning around players. AArsInstancedActorsManager's also provide replication and persistence for their managed instances.
 *
 * Spawned and populated *offline* by UArsInstancedActorsSubsystem::InstanceActor. Offline population ensures client & server
...
public:
	 ...
	 *
	 * Note: By default UArsInstancedActorsSubsystem::RemoveActorInstance will destroy empty managers on
	 ...
	 * @param InstanceTransform If entities have been spawned, this will be taken from the Mass transform fragment, else from UArsInstancedActorsData::InstanceTransforms
	 ...
	 */
	using FInstanceOperationFunc = TFunctionRef<bool(const FArsInstancedActorsInstanceHandle& InstanceHandle, const FTransform& InstanceTransform, FArsInstancedActorsIterationContext& IterationContext)>;

	/**
	 * Predicate taking a UArsInstancedActorsData and returns true if IAD matches search criteria, false otherwise.
	 */
	using FInstancedActorDataPredicateFunc = TFunctionRef<bool(const UArsInstancedActorsData& InstancedActorData)>;

	/**
	 * Call InOperation for each valid instance in this manager. Prior to entity spawning in BeginPlay, this iterates valid UArsInstancedActorsData::InstanceTransforms.
	 * Once entities have been spawned, UArsInstancedActorsData::Entities are iterated.
	 ...
	 * UArsInstancedActorsData::InstanceTransforms. Once entities have been spawned, UArsInstancedActorsData::Entities are iterated.
	...
	template <typename TBoundsType>
	bool ForEachInstance(const TBoundsType& QueryBounds, FInstanceOperationFunc InOperation, FArsInstancedActorsIterationContext& IterationContext
	 ...
	 */
	template <typename TBoundsType>
	static UE::ArsInstancedActors::EInsideBoundsTestResult IsInstanceInsideBounds(const TBoundsType & QueryBounds
	...
	UArsInstancedActorsSubsystem* GetInstancedActorSubsystem() const;
	UArsInstancedActorsSubsystem& GetInstancedActorSubsystemChecked() const;
...
protected:
	// mz@todo a temp glue to be removed once ArsInstancedActors get moved out of original project for good. It's virtual and I don't like it.
...
protected:
	 ...
	 * within this IAM.
	 * @see UArsInstancedActorsData::ID
	 */
	UPROPERTY()
	uint16 NextInstanceDataID = 0;

	/** Per-actor-class instance data populated by AddActorInstance */
	UPROPERTY(Instanced, VisibleAnywhere, Category=ArsInstancedActors)
	TArray<TObjectPtr<UArsInstancedActorsData>> PerActorClassInstanceData;
	...
	TArray<TWeakObjectPtr<UArsInstancedActorsModifierVolumeComponent>> ModifierVolumes;
	...
	TSubclassOf<UArsInstancedActorsData> ArsInstancedActorsDataClass;
...
private:
	...
	UPROPERTY(EditAnywhere, Transient, meta = (DisplayPriority = 1), Category = ArsInstancedActors)
...
// inlines
//-----------------------------------------------------------------------------
FORCEINLINE FArsInstancedActorsManagerHandle AArsInstancedActorsManager::GetManagerHandle() const
{ 
	return ManagerHandle; 
}

FORCEINLINE FBox AArsInstancedActorsManager::GetInstanceBounds() const
{ 
	return InstanceBounds; 
}

FORCEINLINE bool AArsInstancedActorsManager::HasSpawnedEntities() const
{ 
	return bHasSpawnedEntities; 
}

FORCEINLINE TConstArrayView<TObjectPtr<UArsInstancedActorsData>> AArsInstancedActorsManager::GetAllInstanceData() const
{ 
	return PerActorClassInstanceData; 
}

FORCEINLINE TSharedPtr<FMassEntityManager> AArsInstancedActorsManager::GetMassEntityManager() const 
{ 
	return MassEntityManager; 
}

FORCEINLINE FMassEntityManager& AArsInstancedActorsManager::GetMassEntityManagerChecked() const
{
	check(MassEntityManager.IsValid());
	return *MassEntityManager;
}

FORCEINLINE UArsInstancedActorsSubsystem* AArsInstancedActorsManager::GetInstancedActorSubsystem() const 
{ 
	return InstancedActorSubsystem; 
}

FORCEINLINE UArsInstancedActorsSubsystem& AArsInstancedActorsManager::GetInstancedActorSubsystemChecked() const
{
