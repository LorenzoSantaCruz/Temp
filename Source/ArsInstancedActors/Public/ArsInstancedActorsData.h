...
private:
	FExemplarActorData(AActor* InActor, UArsInstancedActorsSubsystem* InInstancedActorSubsystem)
		: Actor(InActor)
		, ArsInstancedActorsSubsystem(InInstancedActorSubsystem)
	{
	}

	UArsInstancedActorsSubsystem* ArsInstancedActorsSubsystem;

	friend class ::UArsInstancedActorsSubsystem;
};
...
public:
	...
	{
		if (const FArsInstancedActorsDataSharedFragment* DataFragment = SharedInstancedActorDataStruct.GetPtr<FArsInstancedActorsDataSharedFragment>())
		{
		...
		return EArsInstancedActorsBulkLOD::MAX;
	}
	...
	const FArsInstancedActorsTagSet& GetAdditionalTags() const
	{
...
protected:
	...
	UPROPERTY(VisibleAnywhere, Category=ArsInstancedActors)
	FArsInstancedActorsTagSet AdditionalTags;
	...
	idea to support UObject properties in settings, so we'll add something like the folowing soon:
		...
		void UArsInstancedActorsData::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
		{
			Super::AddReferencedObjects(InThis, Collector);

			const UArsInstancedActorsData* Data = Cast<UArsInstancedActorsData>(InThis);
			check(Data);
...
public:
	...
	// @see AArsInstancedActorsManager::SpawnInstances
	UPROPERTY(Transient)
	...
	const FArsInstancedActorsVisualizationInfo& GetDefaultVisualizationChecked() const { return GetVisualizationChecked(0); }
	...
	const FArsInstancedActorsVisualizationInfo& GetVisualizationChecked(uint8 VisualizationIndex) const
	{
		...
		const FArsInstancedActorsVisualizationInfo& VisualizationInfo = InstanceVisualizations[VisualizationIndex];
	...
	// @param VisualizationIndex The index of the visualization previously returned by AddVisualization
	const FArsInstancedActorsVisualizationInfo* GetVisualization(uint8 VisualizationIndex) const
	{
	...
	//
	// Note: This is performed via a mass deferred command to add a 'pending' FArsInstancedActorsMeshSwitchFragment
	...
	FMassEntityHandle GetEntityHandleForIndex(const FArsInstancedActorsInstanceIndex Index) const
	{
...
protected:
	...
	UPROPERTY(Transient, VisibleAnywhere, Category=ArsInstancedActors)
	TArray<FArsInstancedActorsVisualizationInfo> InstanceVisualizations;
...
private:
	// Represents the shared fragment registered with MassEntityManager, that points back to this UArsInstancedActorsData instance
	TStructView<FArsInstancedActorsDataSharedFragment> SharedInstancedActorDataStruct;

	// List of FArsInstancedActorsDelta's to apply to instances, replicated via fast array replication to clients.
	...
	FArsInstancedActorsDeltaList InstanceDeltas;

	UPROPERTY(Transient)
	FMassEntityConfig EntityConfig;

	// Result of combining AdditionalTags with class-based FArsInstancedActorsSettings::GameplayTags
	UPROPERTY(Transient)
	FGameplayTagContainer CombinedTags;

	bool bCanHydrate = true;

	TSharedPtr<UE::ArsInstancedActors::FExemplarActorData> ExemplarActorData;

	struct FSetReplicatedActorRequests
	{
		FSetReplicatedActorRequests(FArsInstancedActorsInstanceIndex InInstanceIndex, TWeakObjectPtr<AActor> InActor)
...
// INLINES
//-----------------------------------------------------------------------------
inline bool UArsInstancedActorsData::CanHydrate() const
{
	return bCanHydrate;
}
