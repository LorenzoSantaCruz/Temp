...
public:
	 ...
	 * UArsInstancedActorsComponent's an opportunity to extend Mass entity default traits.
	 * 
	 * Note: The `exemplar' actor is an actor spawned into a separate inactive UWorld by UArsInstancedActorsSubsystem::GetOrCreateExemplarActor
	 ...
	 */
	virtual void ModifyMassEntityConfig(FMassEntityManager& InMassEntityManager, UArsInstancedActorsData* InstancedActorData, FMassEntityConfig& InOutMassEntityConfig) const {}

	/** 
	 * Called on ActorClass CDO components for clients & servers during UArsInstancedActorsData::CreateEntityTemplate to provide 
	 * UArsInstancedActorsComponent's an opportunity to extend Mass entity default fragments.
	 *
	 ...
	 */
	virtual void ModifyMassEntityTemplate(FMassEntityManager& InMassEntityManager, UArsInstancedActorsData* InstancedActorData, FMassEntityTemplateData& InOutMassEntityTemplateData) const {}
	 ...
	 * match serialized persistence records with this UArsInstancedActorsComponent's SerializeInstancePersistenceData implementation.
	 */
	 ...
	 */
	virtual bool ShouldSerializeInstancePersistenceData(const FArchive& Archive, UArsInstancedActorsData* InstanceData, int64 TimeDelta) const { return false; }

	/**
	 * Called by AArsInstancedActorsManager::SerializeInstancePersistenceData for IAD's with an ActorClass containing this UArsInstancedActorsComponent,
	 ...
	 */
	virtual void SerializeInstancePersistenceData(FStructuredArchive::FRecord Record, UArsInstancedActorsData* InstanceData, int64 TimeDelta) const {}
...
protected:
	...
	* On servers, InstanceHandle will be set in UArsInstancedActorsComponent::InitializeComponent (before component BeginPlay)
	* On clients, InstanceHandle will be set after component BeginPlay.
	*/
	UPROPERTY(ReplicatedUsing=OnRep_InstanceHandle, Transient, Meta=(AllowPrivateAccess="true"), BlueprintReadOnly, Category=ArsInstancedActors)
...
template<typename FragmentType>
void UArsInstancedActorsComponent::AddFragmentDeferred()
{
...
template<typename... FragmentTypes>
void UArsInstancedActorsComponent::AddOrUpdateFragmentsDeferred(FragmentTypes&&... NewFragments)
{
	FMassEntityHandle MassEntityHandle = GetMassEntityHandle();
	if (ensure(MassEntityHandle.IsValid()))
	{
...
template<typename... FragmentTypes>
void UArsInstancedActorsComponent::AddOrUpdatePersistentFragmentsDeferred(FragmentTypes&&... NewFragments)
{
	FMassEntityHandle MassEntityHandle = GetMassEntityHandle();
	if (ensure(MassEntityHandle.IsValid()))
	{
...
template<typename FragmentType>
const FragmentType* UArsInstancedActorsComponent::GetVolatileFragment() const
{
	FMassEntityHandle MassEntityHandle = GetMassEntityHandle();
	if (ensure(MassEntityHandle.IsValid()))
	{
...
template<typename FragmentType>
void UArsInstancedActorsComponent::RemoveFragmentDeferred()
{
	FMassEntityHandle MassEntityHandle = GetMassEntityHandle();
	if (ensure(MassEntityHandle.IsValid()))
	{
